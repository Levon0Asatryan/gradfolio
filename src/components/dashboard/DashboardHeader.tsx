...
const DashboardHeader: FC<DashboardHeaderProps> = ({ user = DEFAULT_USER, stats, engagementRate = 3.7, onEditProfile, onViewAnalytics }) => {
  return (
    <Card
      component="section"
      ...
      sx={{
        ...
        "&:hover": { boxShadow: 6 },
        backgroundImage: (t) => `linear-gradient(180deg, ${t.palette.action.hover} 0%, transparent 100%)`,
      }}
    >
      <CardContent>
        <Grid container spacing={2} columns={{ xs: 12, md: 12 }}>
          ...
          <Grid size={{ xs: 12, md: 4 }}>
            <Grid container spacing={1} columns={{ xs: 12 }}>
              <Grid size={{ xs: 6, sm: 6 }}>
                <Chip icon={<VisibilityIcon />} label={`${stats?.profileViews ?? 0} views`} variant="outlined" sx={{ width: "100%" }} />
              </Grid>
              <Grid size={{ xs: 6, sm: 6 }}>
                <Chip icon={<AnalyticsIcon />} label={`${engagementRate.toFixed(1)}% engagement`} color="info" variant="outlined" sx={{ width: "100%" }} />
              </Grid>
              <Grid size={{ xs: 6, sm: 6 }}>
                <Chip icon={<ArticleIcon />} label={`${stats?.totalPublications ?? 0} publications`} variant="outlined" sx={{ width: "100%" }} />
              </Grid>
              <Grid size={{ xs: 6, sm: 6 }}>
                <Chip icon={<FolderIcon />} label={`${stats?.totalProjects ?? 0} projects`} variant="outlined" sx={{ width: "100%" }} />
              </Grid>
            </Grid>
          </Grid>
        </Grid>
        ...
        <Stack direction="row" spacing={1} sx={{ mt: 1 }}>
          <Button startIcon={<EditIcon />} variant="outlined" onClick={onEditProfile} sx={{ height: 40 }}>
            Edit Profile
          </Button>
          <Button startIcon={<AnalyticsIcon />} variant="contained" onClick={onViewAnalytics} sx={{ height: 40 }}>
            View Analytics
          </Button>
        </Stack>
      </CardContent>
